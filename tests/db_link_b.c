...
int list_link_bs(db *db, MDB_txn *ptxn, size_t *out_len,
                 db_link_b **out_items) {
  MDB_txn *txn;
  int rc;
  MDB_val key, data;
  ...
  *out_items = NULL;
  ...
  rc = mdb_stat(txn, db->link_b, &stat);
  if (rc) {
    fprintf(stderr, "Failed to get database stat: %s\n", mdb_strerror(rc));
    goto _defer_1;
  }
  out_data = (db_link_b *)malloc(stat.ms_entries * sizeof(db_link_b));
  if (!out_data) {
    fprintf(stderr, "Failed to allocate memory\n");
    goto _defer_1;
  }
  memset(out_data, 0, stat.ms_entries * sizeof(db_link_b));

  rc = mdb_cursor_open(txn, db->link_b, &cursor);
  if (rc) {
...
int put_link_b(db *db, MDB_txn *ptxn, db_link_b *in_out_item) {
  ...
  if (id == 0) {
    // new item
    rc = db_new_pk(db, PK_link_b, txn, &id);
    if (rc)
      goto _defer_1;
  ...
  key = (MDB_val){sizeof(uint64_t), &id};
  data = (MDB_val){sizeof(db_link_b), in_out_item};
...
int delete_link_b(db *db, MDB_txn *ptxn, uint64_t item_id,
                  db_link_b *out_item) {
  MDB_txn *txn;
  int rc;
  MDB_val key, data;
  ...
  rc = mdb_del(txn, db->link_b, &key, &data);
  if (rc) {
  ...
  if (out_item->link_id != 0) {
    ...
    if (rc) {
      fprintf(stderr, "Failed to update link_as: %s\n", mdb_strerror(rc));
      goto _defer_1;
    }
    db_link_a link_item;
    ...
    if (link_item.link_id == out_item->link_id) {
      ...
      data = (MDB_val){sizeof(db_link_a), &link_item};
