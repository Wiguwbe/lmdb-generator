...
int list_bastards(db *db, MDB_txn *ptxn, size_t *out_len,
  ...
  *out_items = NULL;
  ...
  rc = mdb_stat(txn, db->bastard, &stat);
  if (rc) {
    fprintf(stderr, "Failed to get database stat: %s\n", mdb_strerror(rc));
    goto _defer_1;
  }
  out_data = (db_bastard *)malloc(stat.ms_entries * sizeof(db_bastard));
  if (!out_data) {
    fprintf(stderr, "Failed to allocate memory\n");
    goto _defer_1;
  }
  memset(out_data, 0, stat.ms_entries * sizeof(db_bastard));

  rc = mdb_cursor_open(txn, db->bastard, &cursor);
  if (rc) {
...
int get_bastard(db *db, MDB_txn *ptxn, uint64_t item_id, db_bastard *out_item) {
  ...
  rc = mdb_get(txn, db->bastard, &key, &data);
  if (rc) {
...
int put_bastard(db *db, MDB_txn *ptxn, db_bastard *in_out_item) {
  ...
  if (id == 0) {
    // new item
    rc = db_new_pk(db, PK_bastard, txn, &id);
    if (rc)
      goto _defer_1;
  ...
  key = (MDB_val){sizeof(uint64_t), &id};
  data = (MDB_val){sizeof(db_bastard), in_out_item};
  rc = mdb_put(txn, db->bastard, &key, &data, 0);
  if (rc) {
...
int delete_bastard(db *db, MDB_txn *ptxn, uint64_t item_id,
  ...
  rc = mdb_del(txn, db->bastard, &key, &data);
  if (rc) {
