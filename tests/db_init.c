...
int db_init(db *db, char *path) {
  ...
  rc = mdb_env_set_maxdbs(db->env, (MDB_dbi)8);
  if (rc) {
  ...
  struct _dbi_name dbi_name_array[] = {
      ...
      {"plain", &db->plain, MDB_CREATE | MDB_INTEGERKEY},

      {"link_a", &db->link_a, MDB_CREATE | MDB_INTEGERKEY},

      {"link_b", &db->link_b, MDB_CREATE | MDB_INTEGERKEY},

      {"parent", &db->parent, MDB_CREATE | MDB_INTEGERKEY},

      {"child", &db->child, MDB_CREATE | MDB_INTEGERKEY},

      {"bastard", &db->bastard, MDB_CREATE | MDB_INTEGERKEY},

      {"parent_children", &db->parent_children,
...
void db_close(db *db) { mdb_env_close(db->env); }
