...
int list_link_as(db *db, MDB_txn *ptxn, size_t *out_len,
                 db_link_a **out_items) {
  MDB_txn *txn;
  int rc;
  MDB_val key, data;
  ...
  *out_items = NULL;
  ...
  rc = mdb_stat(txn, db->link_a, &stat);
  if (rc) {
    fprintf(stderr, "Failed to get database stat: %s\n", mdb_strerror(rc));
    goto _defer_1;
  }
  out_data = (db_link_a *)malloc(stat.ms_entries * sizeof(db_link_a));
  if (!out_data) {
    fprintf(stderr, "Failed to allocate memory\n");
    goto _defer_1;
  }
  memset(out_data, 0, stat.ms_entries * sizeof(db_link_a));

  rc = mdb_cursor_open(txn, db->link_a, &cursor);
  if (rc) {
...
int get_link_a(db *db, MDB_txn *ptxn, uint64_t item_id, db_link_a *out_item) {
  ...
  rc = mdb_get(txn, db->link_a, &key, &data);
  if (rc) {
...
int put_link_a(db *db, MDB_txn *ptxn, db_link_a *in_out_item) {
  ...
  if (id == 0) {
    // new item
    rc = db_new_pk(db, PK_link_a, txn, &id);
    if (rc)
      goto _defer_1;
  ...
  key = (MDB_val){sizeof(uint64_t), &id};
  data = (MDB_val){sizeof(db_link_a), in_out_item};
  rc = mdb_put(txn, db->link_a, &key, &data, 0);
  if (rc) {
...
int delete_link_a(db *db, MDB_txn *ptxn, uint64_t item_id,
                  db_link_a *out_item) {
  MDB_txn *txn;
  int rc;
  MDB_val key, data;
  ...
  rc = mdb_del(txn, db->link_a, &key, &data);
  if (rc) {
  ...
  if (out_item->link_id != 0) {
    key = (MDB_val){sizeof(uint64_t), &out_item->link_id};
    rc = mdb_get(txn, db->link_b, &key, &data);
    if (rc) {
      fprintf(stderr, "Failed to update link_bs: %s\n", mdb_strerror(rc));
      goto _defer_1;
    }
    db_link_b link_item;
    memcpy(&link_item, data.mv_data, data.mv_size);
    if (link_item.link_id == out_item->link_id) {
      link_item.link_id = 0;
      key = (MDB_val){sizeof(uint64_t), &link_item.id};
      data = (MDB_val){sizeof(db_link_b), &link_item};
      rc = mdb_put(txn, db->link_b, &key, &data, 0);
      if (rc) {
        fprintf(stderr, "Failed to update link_bs: %s\n", mdb_strerror(rc));
        goto _defer_1;
      }
